...
namespace App\Http\Controllers\Api\Event\Workshop;
...
class WorkshopsController extends Controller
{
    ...
    {
        $years = SubCategory::where('category_id',2)->orderBy('title', 'ASC')->get(['title','sub_category_id']);
        $all_workshops = [];
        foreach($years as $y){
            $workshops = Events::where('publish',1)->where('deleted',0)->where('sub_category_id',$y->sub_category_id)->orderBy('start_date','ASC')->get(['event_id','title_en','location_en','start_date','end_date']);
            $yc = [];
            $yc['upcomming'] = [];
            $yc['overview'] = [];
            foreach($workshops as $co) {
                $c = [];
                $c['event_id'] = $co->event_id;
                $c['title'] = $co->title_en;
                $c['location'] = $co->location_en;
                $c['image'] = url('/storage/uploads/workshops/'.$co->event_id.'/cover_img.jpg');
            ...
            $all_workshops[$y->title] = $yc;
        }
        return response()->json($all_workshops);
    }

        public function show($event_id)
    {
        $event_base = Events::where('event_id', $event_id)->where('category_id',2)->where('publish',1)->where('deleted',0)->first([
        ...
        if($event_base != null){
            ...
            $event_base['image'] = url('/storage/uploads/workshops/'.$event_id.'/cover_img.jpg');
            ...
            $event_base['call'] = 'IEREK Workshop';
