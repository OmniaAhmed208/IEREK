...
class WorkshopFeesController extends Controller
{
    //
    public function show($event_id)
    {
        ...
        return view('admin.events.workshop.fees.show')->with(
            array(
            'fees' => $fees, 
            'event_id' => $event_id,
            'event' => $eventTitle,
    ...
    public function filter($event_id, $deleted)
    {
        ...
        switch($deleted)
        {
            case "active-only";
                ...
                return View('admin.events.workshop.fees.show')->with(array('fees' => $fees) )->with('select','active-only')->with('event_id', $event_id)->with(
            ...
            case "inactive-only";
                ...
                return View('admin.events.workshop.fees.show')->with(array('fees' => $fees) )->with('select','inactive-only')->with('event_id', $event_id)->with(
            'event_currency', $event['currency'])->with(array(
            'event' => $eventTitle
            ));
            break;
            case "all";
                ...
                return View('admin.events.workshop.fees.show')->with(array('fees' => $fees) )->with('select','all')->with('event_id', $event_id)->with(
            'event_currency', $event['currency'])->with(array(
            'event' => $eventTitle
            ));
            break;
    ...
    public function create($event_id)
    {
        ...
        $event = Events::where('event_id', $event_id)->firstOrFail();
        $eventTitle = $event['title_en'];
        return view('admin.events.workshop.fees.create')->with(
            array(
            'for' => $for, 
            'condition' => $condition, 
            'category' => $category, 
    ...
    public function edit($id)
    {
        ...
        $event = Events::where('event_id', $event_id)->firstOrFail();
        $eventTitle = $event['title_en'];
        return view('admin.events.workshop.fees.edit')->with(
            array(
                'fee' => $fees,
                'for' => $for, 
                'condition' => $condition, 
