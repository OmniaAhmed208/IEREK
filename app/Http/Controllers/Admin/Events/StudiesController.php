...
class StudiesController extends Controller
{
    ...
    public function index()
    {
        ...
        $supers = Events::where('category_id',$this->category_id)->get();
        $groupArray = [];
        ...
        $fStudies = Events::where('deleted', '=', 0)->where('category_id',$this->category_id)->whereIn('event_id', $groupArray)->get();

        return View('admin.events.studies.index')->with(array('studies' => $fStudies));
    }
    ...
    public function filter($deleted)
    {
        ...
        {
            case "active-only";
                $fStudies = Events::whereIn('event_id', $groupArray)->where('deleted', '=', 0)->where('category_id',$this->category_id)->get();
                return View('admin.events.studies.index')->with(array('studies' => $fStudies) )->with('select','active-only');
            break;
            case "inactive-only";
                $fStudies = Events::whereIn('event_id', $groupArray)->where('deleted', '=', 1)->where('category_id',$this->category_id)->get();
                return View('admin.events.studies.index')->with(array('studies' => $fStudies) )->with('select','inactive-only');
            break;
            case "all";
                    $fStudies = Events::where('category_id',$this->category_id)->whereIn('event_id', $groupArray)->get();

                return View('admin.events.studies.index')->with(array('studies' => $fStudies) )->with('select','all');
            break;
        }
    }
    ...
    public function create()
    {
        ...
        return View('admin.events.studies.create')->with(array('eventmasters' => $eventmasters));
        }else{
    ...
    // validating submitted data
        ...
        else
        {
            ...
            $ifSubCategory = SubCategory::where('title', $sYear)->where('category_id',$this->category_id)->first();
            ...
            if($subCategoryId !== null)
            {
                ...
                for ($x = 0; $x < $cusers; $x++) {
                    ...
                    $notification = Notifications::create(array(
                        'title' => 'New Studies Created By '.$createdBy,
                    ...
                    '<:Meta Description:>' => '&&'.@$data['meta_description'].'##'
                        ...
                        'icon' => 'bullhorn',
                        'timeout' => 5000,
                        'url' => '/admin/events/studies/'.$id.'/edit',
                        'status' => 'info'
                    ));
                }
            }
            ...
            if ($request->hasFile('writing_template')  &&  $request->file('writing_template')->isValid()) {
                ...
                $destinationPath = 'uploads/studies/'.$id; // upload path
    ...
    public function edit($id)
    {
        ...
        $sub_categories = SubCategory::where('category_id', $this->category_id)->get();
        $url1 = asset('storage/uploads/studies/'.$id.'/cover_img.jpg');
        $url2 = asset('storage/uploads/studies/'.$id.'/list_img.jpg');
        $url3 = asset('storage/uploads/studies/'.$id.'/slider_img.jpg');
        $url4 = asset('storage/uploads/studies/'.$id.'/featured_img.jpg');
        $url = array(
            'cover'     => $url1,
            'list'      => $url2,
            'slider'    => $url3,
        ...
        return View('admin.events.studies.edit')->with('sub_categories', $sub_categories)->with('event', $event)->with('url', $url)->with('eventmasters' ,$eventmasters);
        }else{
            return view('errors.404');
        }
    }

    //update studies general
    ...
    // validating submitted data
        ...
        else
        {
            ...
            //update Studies
            // Store the updated studies data...
            ...
            for ($x = 0; $x < $cusers; $x++) {
                ...
                $notification = Notifications::create(array(
                    'title' => 'Studies (General Information) Edited By '.$createdBy,
                    'description' => json_encode(array(
    ...
    public function destroy($id)
    {
        ...
        for ($x = 0; $x < $cusers; $x++) {
            ...
            $notification = Notifications::create(array(
                'title' => 'Studies deleted By '.$createdBy,
                'description' => 'User '.$createdBy.' deleted studies',
                'user_id' => $users[$x]['user_id'],
                'color' => 'red',
                'type' => 'studies-edit',
                'icon' => 'times',
        ...
        if($delete == 1)
        {
            return redirect('admin/events/studies');
        }
        }else{
            return view('errors.404');
        }
    ...
    public function restore($id)
    {
        ...
        for ($x = 0; $x < $cusers; $x++) {
            ...
            $notification = Notifications::create(array(
                'title' => 'Studies restored By '.$createdBy,
                'description' => 'User '.$createdBy.' restored studies',
                'user_id' => $users[$x]['user_id'],
                'color' => 'green',
                'type' => 'studies-edit',
                'icon' => 'undo',
