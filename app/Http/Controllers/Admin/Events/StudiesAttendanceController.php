...
class StudiesAttendanceController extends Controller
{
    //
	public function filter($event_id, $deleted)
    {
        ...
        switch($deleted)
        {
            case "active-only";
                ...
                return View('admin.events.studies.attendance.index')->with(array(
            		'event' => $eventTitle,
    ...
    public function export(Request $request)
    {
        ...
        switch($filter)
        {
            case 'all':
                $event = Events::where('event_id',$event_id)->first();
                $attendance = EventAttendance::where('event_id',$event_id)->get();
                return View('admin.events.studies.attendance.export')->with(array(
                    'attendances' => $attendance,
                    'type' => 'All',
                    'event' => $event
                ));
    ...
    public function application($event_id, $user_id)
    {
        $app = StudyApplication::where('event_id',$event_id)->where('user_id',$user_id)->first();

        return view('admin.events.studies.attendance.app')->with(['data' => $app]);
    }
}
