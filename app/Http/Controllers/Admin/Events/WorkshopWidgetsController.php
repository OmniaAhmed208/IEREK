...
class WorkshopWidgetsController extends Controller
{
    //
    public function show($event_id)
    {
        ...
        return view('admin.events.workshop.widgets.show')->with(
            array(
            'widgets' => $widgets, 
            'event_id' => $event_id,
            'event' => $eventTitle
    ...
    public function filter($event_id, $deleted)
    {
        ...
        switch($deleted)
        {
            case "active-only";
                $widgets = EventWidgets::where('event_id','=', $event_id)->where('deleted', '=', 0)->get();
                return View('admin.events.workshop.widgets.show')->with(array('widgets' => $widgets) )->with('select','active-only')->with('event_id', $event_id)->with(array(
            'event' => $eventTitle));
            break;
            case "inactive-only";
                $widgets = EventWidgets::where('event_id','=', $event_id)->where('deleted', '=', 1)->get();
                return View('admin.events.workshop.widgets.show')->with(array('widgets' => $widgets) )->with('select','inactive-only')->with('event_id', $event_id)->with(array(
            'event' => $eventTitle));
            break;
            case "all";
                $widgets = EventWidgets::where('event_id','=', $event_id)->get();
                return View('admin.events.workshop.widgets.show')->with(array('widgets' => $widgets) )->with('select','all')->with('event_id', $event_id)->with(array(
            'event' => $eventTitle));
            break;
        }
    }
    ...
    public function create($event_id)
    {
        ...
        $event = Events::where('event_id', $event_id)->firstOrFail();
        $eventTitle = $event['title_en'];
        return view('admin.events.workshop.widgets.create')->with(
            array(
            'for' => $for, 
            'condition' => $condition, 
    ...
    public function edit($id)
    {
        ...
        $url1 = Storage::disk('public')->url('uploads/workshops/'.$event_id.'/'.'widgets/'.$eventWidget->img);

        $url = array(
            'widget_img'     => $url1
            );

        return view('admin.events.workshop.widgets.edit')->with(
            array(
                'event_id' => $event_id,
                'event' => $event,
        ...
        else
        {
            ...
            if($id)
            {
                ...
                if ($request->hasFile('widget_img')  &&  $request->file('widget_img')->isValid()) 
                {
                    ...
                    $destinationPath = 'uploads/workshops/'.$event_id.'/'.'widgets/'; // upload path
        ...
        else
        {
            ...
            if($idu)
            {
                ...
                if ($request->hasFile('widget_img')  &&  $request->file('widget_img')->isValid()) {
                    ...
                    $destinationPath = 'uploads/workshops/'.$widget->event_id.'/'.'widgets/'; // upload path
