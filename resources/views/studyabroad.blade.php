@extends('layouts.master') @section('content')
    ...
    <div id="CONDETAILP" class="summerSchoolLayout normal-table-layout">
        <div class="container">
            <figure class="cover-img">
                @if(file_exists('storage/uploads/studyabroads/'.$event->event_id.'/cover_img.jpg'))<img
                        src="/storage/uploads/studyabroads/{{ $event->event_id }}/cover_img.jpg" class="img-responsive"
                        alt=""/>@endif
            </figure>
            <div class="row">
			...
			{{--<a href="/study_abroad/application_view/{{ $event->event_id }}/{{$event->category_id}}/{{$event->sub_category_id}}"--}}
            {{--> Application Form </a>--}}
                        {{--</div>--}}

                        <div class="plain-box hidden-sm hidden-xs appForm ">
                            ...
                            @foreach($widgets as $widget)
                                @if($widget->widget_type_id == 1)
                                    <div class="styled-box">
                                        ...
                                        @if($widget->img != '' && file_exists('storage/uploads/studyabroads/'.$widget->event_id.'/widgets/'.$widget->img))
                                            <div><a href="{{$widget->img_url}}"><img
                                                            src="/storage/uploads/studyabroads/{{$widget->event_id}}/widgets/{{$widget->img}}"
                                                            style="width:100%;"></a></div>@endif
                    ...
                    <div class="plain-box hidden-sm hidden-xs">
                        ...
                        <a href="/study_abroad" class="quick-links-btn-mob sidebar-btn">
                            Upcoming Events
                            <i class="fa fa-caret-right"></i>
                        </a>
                        ...
                        <div class="framed-content">
                            <div class="styled-box">
                                <div class="box-content row">
                                    <div class="col-md-3 hidden-xs hidden-sm">
                                        <div class="studyabroad-cover">
                                            <img style="width: 100%; max-height: 218px;"
                                                 src="/storage/uploads/studyabroads/{{ $event->event_id }}/featured_img.jpg">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-3">
                                    <center>
                                        @if(file_exists('storage/uploads/studyabroads/'.$event->event_id.'/slider_img.jpg')) <img src="/storage/uploads/studyabroads/{{ $event->event_id }}/slider_img.jpg" alt="" style="max-width: 100%;  max-height: 232px;" class="hidden-xs hidden-sm" align="center"> @endif
                                    </center>
                                </div>
                                </div>
                            </div>
                                        ...
                                        @if($isreg == 1)
                                            <a href="{{ url('/payment/'.$event->slug) }}" id="regconf" style=""
                                               class="popup-register conference-registration-btn">Fees<span
                                                        class="hidden-xs">To Apply</span> </a>
                                        @else
                        ...
                        <div class="styled-box quick-links-btn-container hidden-md hidden-lg">
                            <div class="plain-box">
                                <button href="/conferences" data-toggle="collapse"
                                        ...
                                        class="quick-links-btn-mob sidebar-btn collapsed" aria-expanded="false">
                            ...
                            <div class="box-content">
                                <section>
                                    ...
                                    @elseif($section->section_type_id == 3)
                                        ...
                                        @endif
                                    @else
                                        <?php if ($section->section_type_id == 7 && file_exists('storage/uploads/workshops/' . $event->event_id . '/featured_img.jpg')) {
                                            echo '<img src="/storage/uploads/workshops/' . $event->event_id . '/featured_img.jpg" class="procedia-right">';
...
@push('scripts')
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            confirmX('<strong>Dear Colleague</strong>,<br>You have successfully registered in this study abroad: <b>{{$event->title_en}}</b>, please confirm your registration as you should pay the required fees to be confirmed.', '{{ url('/payment/'.$event->slug) }}', 'Payment', '<a href="/event/{{ $event->slug }}/postpone">Remind Me Later</a>');
            @endif
            @endif
            @endif
