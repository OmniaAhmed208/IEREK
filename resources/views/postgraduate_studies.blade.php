...
@push('styles')
    <style>
        ...
        input[type=checkbox]{
            position:absolute;width:17px;padding:0;margin:0;height:17px;top:4px
        }
    </style>
@endpush
@php $cd = 0; $ed = 0; @endphp
@section('content')
<div class="container" id="applying-area">
    <figure class="cover-img">
        <img src="/storage/uploads/studies/{{ @$event->event_id }}/cover_img.jpg" alt="{{ @$event->title_en }}"/>
        @if(Auth::check())
            @if($isreg == 1)
                <a href="/payment/{{ @$event->slug }}" id="regconf"  class="popup-register conference-registration-btn">Fees<span class="hidden-xs">Payment</span> </a>
            @else
                <a href="javascript:void(0);" onclick="showSection('register','register')" id="regconf" class="popup-register conference-registration-btn"><i class="fa fa-graduation-cap"></i> Apply<span class="hidden-xs">For Students</span> </a>
            @endif
        @else
            <a href="javascript:void(0);" onclick="display_log(0)" id="regconf" class="popup-register conference-registration-btn"><i class="fa fa-graduation-cap"></i> Login<span class="hidden-xs">For Apply</span> </a>
        @endif
    </figure>
    <div class="row">
        <div class="col-md-3">
        ...
        <div class="col-md-9 no-content-styles">
            @foreach($sections as $section)
            <div style="overflow: hidden;margin-top:0px" class="styled-box section" <?php if($section->section_type_id != 1){echo 'style="display:none"';} ?> id="{{ strtolower(str_replace(' ', '-', preg_replace('/[^A-Za-z0-9\-]/', '', $section->title_en))) }}">
                <div class="box-title">
            ...
            <div style="overflow: hidden;margin-top:0px;display:none" class="styled-box section" id="register">
                <form name="application" id="application" action="/study_abroad_categories/register" method="post">
                    {{ csrf_field() }}
                    ...
                    <div class="form-group">
                        <input type="text" name="field" class="form-control requiredz" placeholder="Specialty/Field of Study">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="apply_year" class="form-control" placeholder="YYYY-MM-DD">
                    </div>
                    <hr class="main-hr">
                    ...
                    <div class="form-group">
                        <input type="text" name="full_name" class="form-control requiredz" placeholder="Full Name" value="{{ @$data = auth()->user()->first_name.' '.@$data = auth()->user()->last_name }}">
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-6">
                                <select name="gender" class="form-control requiredz">
                                    <option value="0" {{ @$data = auth()->user()->gender === 0 ? 'selected disabled' : '' }} >Gender</option>
                                    <option value="Male" {{ @$data = auth()->user()->gender === 1 ? 'selected' : '' }}>Male</option>
                                    <option value="Female" {{ @$data = auth()->user()->gender === 2 ? 'selected' : '' }}>Female</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="birth_date" class="form-control" placeholder="Date of Birth" value="{{ @$data = auth()->user()->age }}">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-6">
                                <input type="text" name="nationality" class="form-control requiredz" placeholder="Nationality" value="{{ @$data = auth()->user()->countries->name }}">
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="pp_no" class="form-control" placeholder="Passport No." >
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-6">
                                <input type="text" name="pp_issue" class="form-control" placeholder="Passport Issue Date">
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="pp_expire" class="form-control" placeholder="Passport Expire Date" >
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-6">
                                <input type="text" name="phone" class="form-control requiredz" placeholder="Phone with country code" value="{{ @$data = auth()->user()->phone }}">
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="email" class="form-control requiredz" placeholder="Email" value="{{ @$data = auth()->user()->email }}">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <input type="text" name="where" class="form-control" placeholder="Where? (please specify)">
                    </div>
                    <hr class="main-hr">
                    ...
                    <div class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <input type="text" name="from" class="form-control" placeholder="FROM YYYY-MM-DD">
                            </div>
                            <div class="col-md-8">
                                <input type="text" name="to" class="form-control" placeholder="TO YYYY-MM-DD">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <input type="text" name="school_cert" class="form-control" placeholder="Received Certificate">
                    </div>
                    <div class="form-group">
                        <label>English Level</label>
                        <div class="row">
                            <div class="col-md-6">
                                <input type="text" name="toefl_level" class="form-control" placeholder="TOEFL Level">
                            </div>
                            <div class="col-md-6">
                                <input type="text" name="ielts_level" class="form-control" placeholder="IELTS Level">
                            </div>
                        </div>
                    </div>
                    <hr class="main-hr">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-4">
                                <input type="text" name="college" class="form-control" placeholder="Name">
                            </div>
                            <div class="col-md-8">
                                <input type="text" name="college_address" class="form-control" placeholder="Address">
                            </div>
                        </div>
                    </div>
                    <div class="form-group">
                        <input type="text" name="college_cert" class="form-control" placeholder="Received Certificate">
                    </div>
                    <hr class="main-hr">
                    ...
                    <div class="form-group">
                        <textarea name="study_location" class="form-control" placeholder="Which geographic area or country would you like to study in?"></textarea>
                    </div>
                    <div class="form-group">
                        <textarea name="study_budget" class="form-control" placeholder="How much budget per year can you afford?"></textarea>
                    </div>
                    <div class="form-group">
                        <textarea name="study_domain" class="form-control" placeholder="Which domain of study you like?"></textarea>
                    </div>
                    <div class="form-group text-center">
                        <button type="submit" onclick="submitApplication(event);" class="btn btn-primary btn-lg undergrad_studies_btn"><i class="fa fa-graduation-cap"></i> Submit My Application</button>
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if(response.success === true){
                        window.location.href = "/study_abroad_categories/postgraduate-studies#applied";
                        location.reload();
                    }else{
