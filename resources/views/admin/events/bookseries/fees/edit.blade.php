...
@section('return-url'){{route('showBook SeriesFees', $event_id)}}@endsection
@section('panel-title')Update Book Series Fees <small>{{ $event }}</small>
@endsection
@section('content')
...
$(document).ready( function() {
    $('#update').on('click', function(event) {
        ...
        type: 'POST',
        url: '{{ route("updateBook SeriesFees", $fee->event_fees_id) }}',
        data: formData,
        dataType: 'json',
        cache: false,
        contentType: false,
        processData: false,
            ...
            success: function (response) {
                ...
                else
                {
                    ...
                    setTimeout(
                        function(){
                            window.open('/admin/events/bookseries/fees/{{ $event_id }}', '_self');
                            $('#success').fadeOut(200);
                        }
                    ,1000);
                }
    ...
    <div class="row">
                ...
                <div class="panel panel-default">                            
                    <div class="panel-body tab-content">
                        <div class="tab-pane fade in active" id="tab-General">
                            <p>Update bookseries fees information.</p>
                            ...
                            <div class="form-group" id="fees_category_id">
                                ...
                                <div class="col-md-7 col-xs-12" data-toggle="tooltip" data-placement="left" title="Choose the category of payment">         
                                    <select name="fees_category_id" class="form-control select">
                                        <option @if($fee->fees_category_id == $category[0]->fees_category_id) {{ 'selected' }} @endif value="{{ $category[0]->fees_category_id }}">{{ $category[0]->title }}</option>
                                        <option @if($fee->fees_category_id == $category[5]->fees_category_id) {{ 'selected' }} @endif value="{{ $category[5]->fees_category_id }}">{{ $category[5]->title }}</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="col-md-7 col-xs-12">
                                    <div class="row">
                                        <div class="col-md-9 row" style="padding-left: 0;">                                    
                                            <input type="number" step="any" min="0" class="form-control" value="{{$fee->amount}}" name="amount" placeholder="" />
                                        ...
                                        <div class="col-md-3 row" style="padding-right: 0;">
                                            <select name="currency" class="form-control select">
                                                <option value="{{$event_currency}}" @if($event_currency == $fee->currency){{'selected'}}@endif>{{$event_currency}}</option>
                                                <option value="EGP" @if($fee->currency == 'EGP'){{'selected'}}@endif>EGP</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div id="success" class="alert alert-success" style="margin-top:1em; display:none;">
                            <span><strong id="welcome">Book Series Fees updated successfully !</strong> </span>
                        </div>
        ...
        <div class="panel-body">
            <table class="table table-stripped">
                <caption>Book Series Fees</caption>
                <thead>
                    <tr>
                        <th>Title</th>
                        <th>Condition</th>
