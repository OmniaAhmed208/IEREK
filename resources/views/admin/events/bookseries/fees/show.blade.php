...
@section('return-url'){{route('indexBook Series')}}@endsection
@section('panel-title')Manage Book Series Fees <small>{{ $event }}</small><br><br><small>Currency: <span style="color:darkgreen;font-weight:700;background:#fff;padding:0.25em;border-radius:5px;">@if($event_currency == ''){{'Not Set'}}@else{{$event_currency}}@endif</span></small>@endsection
@section('content')
        	...
        	<div class="panel panel-default">
                <div class="panel-body">
                	<div class="col-md-2 pull-right">
                		<a href="/admin/events/bookseries/fees/create/{{ $event_id }}" class="btn btn-success btn-block"><span class="fa fa-plus"></span> Add Fees</a>
                	</div>
            		<div class="col-md-3">
                        <form method="get">
                                ...
                                <td>
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu" role="menu">
                                            <li><a href="{{ route('editBook SeriesFees', $afee->event_fees_id) }}" style="cursor:pointer">Edit</a></li>
                                            @if($afee->deleted == 0)
...
<script type="text/javascript">
    $('#deleted').on('change', function(event){
        ...
        window.open('/admin/events/bookseries/fees/{{ $event_id }}/filter/'+deleted, '_self')
    });
        ...
        noty({
            ...
            buttons: [
                    {addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function($noty) {
                        ...
                        $.ajax({
                        type: 'POST',
                        url: '/admin/events/bookseries/fees/restore/'+id,
                        data: {_method: 'post'},
        ...
        noty({
            ...
            buttons: [
                    {addClass: 'btn btn-success btn-clean', text: 'Ok', onClick: function($noty) {
                        ...
                        $.ajax({
                        type: 'POST',
                        url: '/admin/events/bookseries/fees/'+id,
                        data: {_method: 'delete'},
                            beforeSend: function(xhr) {
