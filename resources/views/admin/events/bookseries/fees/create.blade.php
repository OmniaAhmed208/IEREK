@extends('admin.layouts.master') @section('return-url'){{route('showBook SeriesFees', $event_id)}}@endsection @section('panel-title')Create New Fees <small>{{ $event }}</small>@endsection @section('content')
...
$(document).ready(function() {
    $('#create').on('click', function(event) {
        ...
        $.ajax({
            ...
            type: 'POST',
            url: '/admin/events/bookseries/fees/store/{{ $event_id }}',
            data: formData,
            dataType: 'json',
            cache: false,
            contentType: false,
                ...
                <div class="panel panel-default">
                    <div class="panel-body tab-content">
                        <div class="tab-pane fade in active" id="tab-General">
                            <p>Fill in bookseries fees information.</p>
                            ...
                            <div class="form-group" id="fees_category_id">
                                ...
                                <div class="col-md-7 col-xs-12" data-toggle="tooltip" data-placement="left" title="Choose the category of payment">
                                    <select name="fees_category_id" class="form-control select">
                                        <option value="{{ $category[0]->fees_category_id }}">{{ $category[0]->title }}</option>
                                        <option value="{{ $category[5]->fees_category_id }}">{{ $category[5]->title }}</option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div id="success" class="alert alert-success" style="margin-top:1em; display:none;">
                            <span><strong id="welcome">Book Series Fees created successfully !</strong> </span>
                        </div>
